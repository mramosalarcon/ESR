...
public partial class empresa : System.Web.UI.UserControl
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            if (!IsPostBack)
            {
                ...
                // Carga los sectores del cat�logo
                ...
                // Carga el cat�logo de las empresas postulantes
                ...
                // Falta cargar el cat�logo de empresas grandes para seleccionarlas como Cadena de Valor
                ...
                // Obtiene el idEmpresa desde la variable de sesi�n si no viene en el querystring 
                int idEmpresa = this.GetEmpresa();
                if (idEmpresa != 0)
                {
                    // Encontr� el idEmpresa
                    ...
                    if (emp.Cargar())
                    {
                        ...
                        //Aqui recuperar la informaci�n de las certificaciones y ponerlas en el GRID
                        ...
                        // Esto es para poner visible la administraci�n de cuestionarios
                        ...
                        if (_sPerfiles != "")
                        {
                            ...
                            {
                                // Solo esos perfiles permiten asignaci�n de cuestionarios
                                ...
                                if (dsCuestionarios.Tables["Cuestionario"].Rows.Count > 0)
                                {
                                    ...
                                    if (dsCuestionariosEmpresa.Tables["Cuestionario"].Rows.Count > 0)
                                    {
                                        foreach (DataRow row in dsCuestionariosEmpresa.Tables["Cuestionario"].Rows)
                                        {
                                            ...
                                            {
                                                // MRA 13/07/2010
                                                // Asignaci�n autom�tica del prediagn�stico
    ...
    protected void btnFinalizar_Click(object sender, EventArgs e)
    {
        ...
        if (Session["perfil"].ToString().IndexOf('2') == -1 && Session["perfil"].ToString().IndexOf('9') == -1)
        {
            if (txtidEmpresa.Text != "")
            {
                ...
                // Ejecutar un m�todo que cargue los datos del contacto
                // �Para que?
                ...
                // Falta guardar la relaci�n con la empresa y los temas
                if (contacto.GuardaEmpresa())
                {
                    Response.Redirect("~/tools/error.aspx?message=" +
                            "El registro de la empresa se realiz� correctamente, " +
    ...
    protected void btnMasCertificaciones_Click(object sender, EventArgs e)
    {
        if (txtRecoAno.Text != "" && txtRecoInstitucion.Text != "" && txtReco.Text != "")
        {
            ...
            {
                lblCertificaciones.Text += "<tr><td>Error al salvar la certificación.</td><td>";
            }
        }
    }
    ...
    private void GuardarEmpresa()
    {
        ...
        // Esto esta chingon, si el m�todo ya se mand� llamar txtidEmpresa tiene el idEmpresa
        // Si es la primera vez viene vaci� e inserta un nuevo registro
        // Me fum� un churro
        if (txtidEmpresa.Text != "")
        {
        ...
        // Se va a calcular el tama�o de la empresa
        // Dependiendo del tama�o de la empresa se le asigna el cuestionario correspondiente
        if (newEmpresa.Sector != "" && txtNoEmpleados.Text != "")
        {
            ...
            {
                case "Industria":
                    if (newEmpresa.Subsector == "Agr�cola" || newEmpresa.Subsector == "Ganader�a" || newEmpresa.Subsector == "Aprovechamiento forestal" || newEmpresa.Subsector == "Pesca" || newEmpresa.Subsector == "Caza")
                    {
        ...
        //Autorizada para contestar cuestionarios, para las nuevas siempre es falso, ademas de que est� oculto
        newEmpresa.autorizada = rbtAutorizadaSi.Checked;
        // MRA 06/07/2010 23:56
        // Para la nueva versi�n siempre ser� diferente de nulo, pero lo dejamos por si acaso
        // Se manda el idUsuario solo para efectos de bit�cora.
        if (Session["idUsuario"] == null)
            newEmpresa.idUsuario = txtEmail.Text;
        else
            newEmpresa.idUsuario = Session["idUsuario"].ToString();
        // Si todo sale bien al guardar, 
        // se checa si esta visible el updatepanel de administraci�n y se asigna el idEmpresa al txtidEmpresa
        if (newEmpresa.Guardar())
        {
            ...
            // Asignaci�n autom�tica de cuestionario
            else //if (Session["perfil"].ToString().IndexOf('2') == -1 && Session["perfil"].ToString().IndexOf('9') == -1) // Si es administrador y si es staff
            {
                ...
                // Ya qued� lista esta funcionalidad solo hay que tener cuidado si cambian el tama�o les van a quedar
                // asignados los demas cuestionarios, se tendr�n que desasignar.
                //newEmpresa.idCuestionario = Convert.ToInt32(ConfigurationManager.AppSettings["idPrediagnostico"]);
                //newEmpresa.GuardarCuestionario();


                // MRA 22/10/2010 06:18 a.m.
                // Deshabilitar la asignaci�n autom�tica de cuestionario
    ...
    /// <summary>
    /// M�todo para buscar la empresa que pertenece a la cadena de valor de la empresa que se est� registrando
    /// </summary>
